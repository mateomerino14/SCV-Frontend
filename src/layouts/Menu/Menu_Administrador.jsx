...
const styles = {
  ...
  grupo: 'flex flex-col gap-1 pb-1.5 mb-1.5 border-b',
  grupoUltimo: 'flex flex-col gap-1',
  seccionLabel: 'text-xs font-bold font-inter uppercase px-3 pt-2 pb-1 opacity-60',
  opcion: 'flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-colors',
  opcionLabel: 'text-sm font-bold font-inter',
  cerrarBtn: 'flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer mx-4 mb-3 shrink-0',
  cerrarLabel: 'text-sm font-bold font-inter',
  separadorFinal: 'border-t',
}

const seccionAdministracion = [
  { path: '/dashboard/administrador', label: 'Dashboard', icono: LayoutDashboard },
  { path: '/dashboard/administrador/usuarios', label: 'Usuarios', icono: Users },
  { path: '/dashboard/administrador/cargos', label: 'Cargos', icono: Briefcase },
]
...
const seccionPersonal = [
  { path: '/dashboard/empleado', label: 'Viajes Personales', icono: Plane },
  { path: '/dashboard/empleado/historial', label: 'Mis Viajes', icono: Plane },
]

const seccionCuenta = [
  { path: '/dashboard/administrador/perfil', label: 'Perfil', icono: User },
  { path: '/dashboard/administrador/configuracion', label: 'Ajustes', icono: Settings },
]

function MenuAdministrador({ isOpen, onClose, usuario }) {
  ...
  const esActivo = (path) => {
    if (['/dashboard/administrador', '/dashboard/empleado'].includes(path)) {
  ...
  const renderOpciones = (opciones) => opciones.map(({ path, label, icono: Icono }) => (
    ...
    >
      <Icono size={19} style={{ color: esActivo(path) ? COLORS.primary : COLORS.labels }} />
        ...
        <div className={styles.navScroll}>
          <div className={styles.nav}>
            <div className={styles.grupo} style={{ borderColor: COLORS.dataFields }}>
              <p className={styles.seccionLabel} style={{ color: COLORS.labels }}>Administración</p>
              {renderOpciones(seccionAdministracion)}
            </div>
