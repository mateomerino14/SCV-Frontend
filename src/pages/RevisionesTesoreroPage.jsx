...
import useRevisionesTesorero from '../hooks/useRevisionesTesorero'
import useMenu from '../hooks/useMenu'
import useContraseniavencida from '../hooks/useContraseniavencida'
import { getEmpleadosTesorero } from '../services/tesoreroService'
...
function RevisionesTesoreroPage() {
  ...
  } = useRevisionesTesorero()
  const [empleados, setEmpleados] = useState([])

  useEffect(() => {
    getEmpleadosTesorero().then((data) => { if (!data.error) setEmpleados(data) })
  }, [])

  const rutaDetalle = (id) => `/dashboard/tesorero/viaje/${id}`
  const origenDetalle = '/dashboard/tesorero/revisiones'
  ...
  return (
    <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
      ...
      <div className={styles.content}>
        <p className={styles.planLabel} style={{ color: COLORS.title }}>Tesorería</p>
        ...
        <p className={styles.subtitulo} style={{ color: COLORS.text_enviroment_types }}>
          Revisa, aprueba o rechaza los fondos, y consulta el historial de tus decisiones.
        </p>
        ...
        {!loading && viajes.length === 0 && (
          <EmptyState
            ...
            subtitulo={tab === 'PENDIENTES' ? 'No hay viajes esperando aprobación de fondos' : 'No tienes viajes con este estado'}
            icono={
...
export default RevisionesTesoreroPage;