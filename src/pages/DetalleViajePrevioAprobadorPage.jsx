...
import useDetalleViajePrevioAprobador from '../hooks/useDetalleViajePrevioAprobador'
...
function DetalleViajePrevioAprobadorPage() {
  const { id } = useParams()
  const navigate = useNavigate()
  const location = useLocation()
  const origen = location.state?.from || '/dashboard/aprobador/viajes-pendientes'
  ...
  const {
    ...
    editarObservacion,
  } = useDetalleViajePrevioAprobador(id)
  ...
  if (loading) {
    return (
      <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
        ...
        <Navbar text="Aprobación de Viaje" onMenuClick={abrirMenu} fotoPerfil={usuario?.foto_perfil} />
    ...
    return (
      <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 py-12">
          ...
          <p className="text-base font-bold font-inter text-center mt-2" style={{ color: COLORS.text }}>No disponible</p>
  ...
  const esPendiente = viaje.estado === 'APROBADO_VIAJE'
  const puedeAccionar = esPendiente
  const esInternacional = viaje.tipo === 'Internacional'
  const estadoActual = estadoConfig[viaje.estado] || estadoConfig['APROBADO_VIAJE']
      ...
      <div className={styles.content}>
        ...
        {accionCompletada && (
          <p className={styles.exitoBadge} style={{ backgroundColor: accionCompletada === 'EN_CURSO' ? '#d4edda' : '#ffa7a8aa', color: accionCompletada === 'EN_CURSO' ? '#155724' : '#500203' }}>
            {accionCompletada === 'EN_CURSO' ? 'Viaje aprobado. Enviado a revisión de tesorería.' : 'Viaje rechazado correctamente'}
          </p>
        )}
...
export default DetalleViajePrevioAprobadorPage;