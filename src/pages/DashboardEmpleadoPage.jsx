...
import ViajeBorradorCard from '../features/Dashboard_Empleado/ViajeBorradorCard'
...
function DashboardEmpleadoPage() {
  ...
  const {
    viajesBorrador, viajesEnCurso, viajesMostrados, viajesRecientes,
    loading,
    enviandoRevision, errorEnvio, viajeAConfirmar,
    handlePedirEnviarRevision, handleCancelarEnviarRevision, handleConfirmarEnviarRevision,
  } = useDashboard()
  ...
  return (
    <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
      <style>{`
        .scroll-viajes::-webkit-scrollbar { height: 6px; }
        .scroll-viajes::-webkit-scrollbar-track { background: transparent; }
        .scroll-viajes::-webkit-scrollbar-thumb { background: ${COLORS.dataFields}; border-radius: 999px; }
        .scroll-viajes::-webkit-scrollbar-thumb:hover { background: ${COLORS.labels}; }
        .scroll-viajes { scrollbar-width: thin; scrollbar-color: ${COLORS.dataFields} transparent; }
      `}</style>
      <SessionExpiredModal isOpen={sessionExpired} onClose={handleSessionExpiredClose} />
      ...
      <div className={styles.content}>
        ...
        {viajesBorrador.length > 0 ? (
          <div className="relative mb-2">
            <div className={`${styles.scrollRow} scroll-viajes`}>
              {viajesBorrador.map((v) => (
                <div key={v.id_viaje} className="snap-start shrink-0 w-full md:w-[450px] flex flex-col">
                  <ViajeBorradorCard
                    viaje={v}
                    onEnviar={handlePedirEnviarRevision}
                    enviando={enviandoRevision === v.id_viaje}
                  />
                </div>
              ))}
            </div>
          ...
          <div className={styles.emptyRecientes} style={{ background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.title})`, marginBottom: 16 }}>
        ...
        <div className={styles.viajesHeader}>
          ...
          {viajesRecientes.length > 0 && (
            <span className={styles.verTodo} style={{ color: COLORS.secondary }} onClick={() => navigate('/dashboard/empleado/historial')}>
              VER TODO →
            </span>
          )}
        </div>
          ...
          {viajesMostrados.length > 0 ? (
            <div className={styles.recientesInner}>
              {viajesMostrados.map((viaje) => (
                <ViajeRecienteItem key={viaje.id_viaje} viaje={viaje} from="/dashboard/empleado" />
              ))}
            </div>
          ) : (
      ...
      <ConfirmarEnvioModal
        isOpen={!!viajeAConfirmar}
        onClose={handleCancelarEnviarRevision}
        onConfirm={handleConfirmarEnviarRevision}
