...
import useViajesPendientesSupervisor from '../hooks/useViajesPendientesSupervisor'
import useMenu from '../hooks/useMenu'
import useContraseniavencida from '../hooks/useContraseniavencida'
import { getEmpleados, tomarViajePreRevision } from '../services/supervisorService'
...
function ViajesPendientesSupervisorPage() {
  ...
  const {
    ...
    filtros, setFiltros, aplicarFiltros, limpiarFiltros, recargar,
  } = useViajesPendientesSupervisor()
  ...
  const handleTomar = async (id_viaje) => {
    setTomando(id_viaje)
    const data = await tomarViajePreRevision(id_viaje)
    setTomando(null)
    if (data.error) {
            ...
            <div className={styles.grid}>
              {viajesNacionales.map((viaje) => (
                <ViajePreRevisionItem
                  ...
                  origenDetalle="/dashboard/supervisor/viajes-pendientes"
                  onTomar={handleTomar}
...
export default ViajesPendientesSupervisorPage;