...
import { ArrowLeft, AlertTriangle, Download, Eye, Globe, MapPin, Navigation, MessageSquare } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div className={styles.presupuestoBar} style={{ backgroundColor: COLORS.dataFields }}>
        <div className={styles.presupuestoFill} style={{ width: `${porcentaje}%`, backgroundColor: excede ? COLORS.secondary : COLORS.title }} />
      </div>
...
function DetalleRevisorPage() {
  ...
  const exportarExcel = async () => {
    ...
    const todosGastos = [...gastosNacionales, ...gastosInternacionales]
    ...
    const totalIVA = gastosNacionales.reduce((sum, g) => {
      if (!g.Factura) return sum
      return sum + Math.max(0, parseFloat(g.monto_total || 0) - parseFloat(g.Factura?.monto_parcial || 0))
    }, 0)
    const totalRcIva = gastosNacionales.filter(g => g.tipo === 'S').reduce((s, g) => s + parseFloat(g.retencion_rc_iva || 0), 0)
    const totalIue = gastosNacionales.filter(g => g.tipo === 'C').reduce((s, g) => s + parseFloat(g.retencion_iue || 0), 0)
    const totalIt = gastosNacionales.reduce((s, g) => s + parseFloat(g.retencion_it || 0), 0)
    const totalCostoNacional = gastosNacionales.reduce((s, g) => s + parseFloat(g.importe_costo || g.monto_total || 0), 0)
    ...
    merge(8, 1, 8, 10)
    setCelda(8, 1, `FONDO RECIBIDO: Bs. ${fondoRecibido.toFixed(2)}${esInternacional ? `   |   USD ${fondoRecibidoUsd.toFixed(2)}` : ''}`, { bold: true, size: 9, fill: 'D9E1F2', align: 'center', border: borde })
    ...
    const totalCF = gastosNacionales.filter(g => !!g.Factura).reduce((s, g) => s + parseFloat(g.monto_total || 0), 0)
    const totalSF = gastosNacionales.filter(g => !g.Factura).reduce((s, g) => s + parseFloat(g.monto_total || 0), 0)
    const totalIvaRcIva = totalIVA > 0 ? totalIVA : totalRcIva
    ...
    if (esInternacional && gastosInternacionales.length > 0) {
    ...
    const resumen = [
      { label: `Saldo a Devolver (Bs): ${Math.max(0, saldo).toFixed(2)}${esInternacional ? `   |   Saldo a Devolver (USD): ${Math.max(0, saldoUsd).toFixed(2)}` : ''}`, color: colorHeader },
      { label: `Importe a Reembolsar (Bs): ${saldo < 0 ? Math.abs(saldo).toFixed(2) : '0.00'}${esInternacional ? `   |   Importe a Reembolsar (USD): ${saldoUsd < 0 ? Math.abs(saldoUsd).toFixed(2) : '0.00'}` : ''}`, color: (saldo < 0 || saldoUsd < 0) ? colorInter : colorHeader },
    ]
  ...
  const montoLiquidacionUsd = excedePresupuestoUsd
    ? (gastoAcumuladoUsd || 0) - parseFloat(viaje.monto_asignado_usd || 0)
    : parseFloat(viaje.monto_asignado_usd || 0) - (gastoAcumuladoUsd || 0)
  ...
  return (
    <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
      ...
      <div className={styles.content}>
        ...
        {esInternacional && (
          <div className={styles.card} style={{ backgroundColor: COLORS.background }}>
            <PresupuestoBar gastoAcumulado={gastoAcumuladoUsd || 0} montoAsignado={parseFloat(viaje.monto_asignado_usd || 0)} esUsd />
          </div>
        )}
        ...
        <div className={styles.resumenCard} style={{ backgroundColor: COLORS.backgroundHeader }}>
          ...
          <div className={styles.resumenRow}>
            <p className={styles.resumenLabel} style={{ color: COLORS.labels }}>Fondo Recibido (Bs)</p>
            <p className={styles.resumenValor} style={{ color: COLORS.text }}>Bs. {parseFloat(viaje.monto_asignado).toFixed(2)}</p>
          </div>
          <div className={styles.resumenRow}>
            <p className={styles.resumenLabel} style={{ color: COLORS.labels }}>Total Gastado (Bs)</p>
          ...
          <div className={styles.resumenRow}>
            <p className="text-sm font-bold font-inter" style={{ color: excedePresupuesto ? COLORS.secondary : COLORS.title }}>
              {excedePresupuesto ? 'A Reembolsar (Bs)' : 'Saldo a Devolver (Bs)'}
            </p>
            <p className="text-sm font-bold font-inter" style={{ color: excedePresupuesto ? COLORS.secondary : COLORS.title }}>
          ...
          {esInternacional && (
            <>
              <div className={styles.resumenDivider} style={{ borderColor: COLORS.dataFields }} />
              <div className={styles.resumenRow}>
                <p className={styles.resumenLabel} style={{ color: COLORS.labels }}>Fondo Recibido (USD)</p>
                <p className={styles.resumenValor} style={{ color: COLORS.text }}>USD {parseFloat(viaje.monto_asignado_usd || 0).toFixed(2)}</p>
              </div>
              <div className={styles.resumenRow}>
                <p className={styles.resumenLabel} style={{ color: COLORS.labels }}>Total Gastado (USD)</p>
                <p className={styles.resumenValor} style={{ color: COLORS.text }}>USD {(gastoAcumuladoUsd || 0).toFixed(2)}</p>
              </div>
              <div className={styles.resumenDivider} style={{ borderColor: COLORS.dataFields }} />
              <div className={styles.resumenRow}>
                <p className="text-sm font-bold font-inter" style={{ color: excedePresupuestoUsd ? COLORS.secondary : COLORS.primary }}>
                  {excedePresupuestoUsd ? 'A Reembolsar (USD)' : 'Saldo a Devolver (USD)'}
                </p>
                <p className="text-sm font-bold font-inter" style={{ color: excedePresupuestoUsd ? COLORS.secondary : COLORS.primary }}>
                  USD {montoLiquidacionUsd.toFixed(2)}
                </p>
              </div>
            </>
          )}
