...
import useMisViajesPreviosAprobador from '../hooks/useMisViajesPreviosAprobador'
...
const tabsFiltro = [
  { valor: 'APROBADO_VIAJE', label: 'En Proceso' },
  { valor: 'EN_REVISION_TESORERO', label: 'Esperando Fondos' },
  { valor: 'EN_CURSO', label: 'Aprobados' },
  { valor: 'RECHAZADO', label: 'Rechazados' },
]

function MisViajesAprobadorPage() {
  const { menuAbierto, usuario, abrirMenu, cerrarMenu, sessionExpired, handleSessionExpiredClose } = useMenu()
  const { viajes, loading, error, filtroEstado, setFiltroEstado } = useMisViajesPreviosAprobador()
  ...
  return (
    <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
      ...
      <div className={styles.content}>
        ...
        <h1 className={styles.title} style={{ color: COLORS.text }}>Mis Viajes Aprobados</h1>
        <p className={styles.subtitulo} style={{ color: COLORS.labels }}>
          Viajes que revisaste en la fase previa al inicio.
        </p>
            ...
            <div className={styles.grid}>
              {viajesNacionales.map((viaje) => (
                <ViajePreRevisionItem
                  ...
                  origenDetalle="/dashboard/aprobador/viajes-historial"
                />
              ))}
            </div>
          </>
...
export default MisViajesAprobadorPage;