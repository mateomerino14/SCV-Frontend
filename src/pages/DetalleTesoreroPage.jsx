...
import { ArrowLeft, AlertTriangle, Plus, Pencil, Trash2, Pencil as PencilIcon, Globe, MapPin, Navigation } from 'lucide-react'
...
import useDetalleTesorero from '../hooks/useDetalleTesorero'
...
const styles = {
  ...
  montosCard: 'rounded-2xl p-5 mb-4 shadow-md',
  montosHeaderRow: 'flex items-center justify-between mb-3',
  montosTitulo: 'text-xs font-bold font-inter uppercase',
  editarBtn: 'flex items-center gap-1 text-xs font-bold font-inter cursor-pointer',
  montoRow: 'flex flex-col gap-1 mb-3',
  montoLabel: 'text-xs font-bold font-inter uppercase',
  montoValor: 'text-2xl font-bold font-inter',
  montoInput: 'w-full rounded-xl px-4 py-3 border text-lg font-bold font-inter outline-none',
  guardarMontosRow: 'flex gap-2 mt-2',
  guardarMontosBtn: 'flex-1 py-2 rounded-xl font-bold font-nunito text-sm cursor-pointer text-center',
  obsSection: 'mb-4 mt-4',
...
function DetalleTesoreroPage() {
  const { id } = useParams()
  const navigate = useNavigate()
  const location = useLocation()
  const origen = location.state?.from || '/dashboard/tesorero/revisiones'
  ...
  const {
    ...
    montoAsignado, montoAsignadoUsd,
  ...
  if (loading) {
    return (
      <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
        ...
        <Navbar text="Aprobación de Fondos" onMenuClick={abrirMenu} fotoPerfil={usuario?.foto_perfil} />
  ...
  const esPendiente = viaje.estado === 'EN_REVISION_TESORERO'
  const estadoActual = estadoConfig[viaje.estado] || estadoConfig['EN_REVISION_TESORERO']
  const puedeAccionar = esPendiente
  ...
  return (
    <div className={styles.page} style={{ backgroundColor: COLORS.background }}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.montosCard} style={{ backgroundColor: COLORS.background }}>
          <div className={styles.montosHeaderRow}>
            <p className={styles.montosTitulo} style={{ color: COLORS.text_enviroment_types }}>Montos Asignados</p>
            {puedeAccionar && !editandoMontos && (
              <p className={styles.editarBtn} style={{ color: COLORS.secondary }} onClick={() => setEditandoMontos(true)}>
          ...
          {!editandoMontos ? (
            <>
              <div className={styles.montoRow}>
                <p className={styles.montoLabel} style={{ color: COLORS.labels }}>Fondo Nacional (Bs)</p>
                <p className={styles.montoValor} style={{ color: COLORS.title }}>Bs {parseFloat(viaje.monto_asignado).toFixed(2)}</p>
              </div>
              {esInternacional && (
                <div className={styles.montoRow}>
                  <p className={styles.montoLabel} style={{ color: COLORS.labels }}>Fondo Internacional (USD)</p>
                  <p className={styles.montoValor} style={{ color: COLORS.primary }}>USD {parseFloat(viaje.monto_asignado_usd || 0).toFixed(2)}</p>
                </div>
              )}
            </>
          ) : (
            <>
              <div className={styles.montoRow}>
                ...
                <input
                  ...
                  placeholder="0.00"
                  value={montoAsignado}
                  onChange={(e) => handleMontoAsignadoChange(e.target.value)}
                />
              </div>
              {esInternacional && (
                <div className={styles.montoRow}>
                  ...
                  <input
                    ...
                    inputMode="decimal"
                    placeholder="0.00"
                    value={montoAsignadoUsd}
                    onChange={(e) => handleMontoAsignadoUsdChange(e.target.value)}
                  />
                </div>
              )}
              <div className={styles.guardarMontosRow}>
                <button
                  className={styles.guardarMontosBtn}
                  style={{ backgroundColor: guardandoMontos ? COLORS.fields : COLORS.primary, color: COLORS.background }}
                  onClick={handleGuardarMontos}
                  disabled={guardandoMontos}
                >
                  {guardandoMontos ? 'Guardando...' : 'Guardar Montos'}
                </button>
                <button
                  className={styles.guardarMontosBtn}
        ...
        {accionCompletada && (
          <p className={styles.exitoBadge} style={{ backgroundColor: accionCompletada === 'EN_CURSO' ? '#d4edda' : '#ffa7a8aa', color: accionCompletada === 'EN_CURSO' ? '#155724' : '#500203' }}>
            {accionCompletada === 'EN_CURSO' ? 'Fondo aprobado. Confirmación enviada al empleado.' : 'Viaje rechazado correctamente'}
          </p>
        )}
        ...
        {puedeAccionar && !accionCompletada && (
          <div className="mb-4">
            <div className={styles.accionesRow}>
              <button className={styles.accionBtn} style={{ backgroundColor: COLORS.primary, color: COLORS.background }} onClick={() => setShowAprobar(true)}>Aprobar</button>
      ...
      <AgregarComentarioModal isOpen={showAgregarObs} onClose={() => { setShowAgregarObs(false); editarObservacion(0, '') }} onConfirm={handleAgregarComentario} observaciones={observaciones} onEditar={editarObservacion} loading={loadingAccion} error={error} />
...
export default DetalleTesoreroPage;