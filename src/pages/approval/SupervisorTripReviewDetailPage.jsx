...
import useSupervisorPendingTripDetail from '../../hooks/approval/useSupervisorPendingTripDetail';
import useTakeTripReview from '../../features/approval/hooks/useTakeTripReview';
...
const styles = {
  ...
  assignBtn: 'py-2 rounded-xl font-bold font-nunito text-base cursor-pointer text-center w-full',
...
function SupervisorTripReviewDetailPage() {
  const {id} = useParams();
  ...
  } = useSupervisorPendingTripDetail(id);

  const {taking, error: takeError, handleTake} = useTakeTripReview();
  ...
  if (loading) {
    return (
      <div className={styles.page} style={{backgroundColor: COLORS.background}}>
        <Navbar text="Revisión de Viaje" onMenuClick={openMenu} profilePhoto={user?.foto_perfil} />
  ...
  const isMine = trip.id_supervisor_asignado === currentUserId;
      ...
      <div className={styles.content}>
        ...
        {isPending && unassigned && !actionCompleted && (
          <div className={styles.assignWrapper}>
            <p className={styles.assignText} style={{color: COLORS.labels}}>Este viaje no está asignado. Asígnate para poder aprobarlo o rechazarlo.</p>
            <button className={styles.assignBtn} style={{backgroundColor: COLORS.primary, color: COLORS.background, opacity: taking ? 0.7 : 1}}
              onClick={() => handleTake(id)} disabled={taking}>
              {taking ? 'Asignando...' : 'Asignarme este viaje'}
            </button>
          </div>
        )}
...
export default SupervisorTripReviewDetailPage;