...
import { getViajesPendientesAprobador, getMisViajesAprobador } from '../services/aprobadorService'

const POLLING_INTERVAL = 30 * 1000

function useRevisionesAprobador() {
  ...
  const cargar = async (f = filtrosRef.current, mostrarLoading = true) => {
    ...
    const [pend, mios] = await Promise.all([
      getViajesPendientesAprobador(f),
      getMisViajesAprobador(),
    ])
  ...
  const viajesAprobados = misViajes.filter(v => ['EN_REVISION_TESORERO', 'EN_CURSO'].includes(v.estado))
...
export default useRevisionesAprobador;