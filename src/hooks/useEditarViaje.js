...
import { obtenerDetalleViaje, editarViaje } from '../services/dashboardService'

function useEditarViaje(id_viaje, usuario) {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      setFechaInicio(v.fecha_inicio || '')
      setFechaFin(v.fecha_fin || '')
      setTipo(v.tipo || 'Nacional')
      setTransporte(v.transporte || 'Terrestre')
      setEstadoOriginal(v.estado || null)
    }
    cargar()
  }, [id_viaje])
  ...
  const handleGuardar = async (onExito) => {
    const errores = validar()
    if (Object.keys(errores).length > 0) { setErroresCampo(errores); return }
    setErroresCampo({})
    setLoading(true)
    const data = await editarViaje(id_viaje, {
      motivo: motivo.trim(),
      origen: origen.trim(),
