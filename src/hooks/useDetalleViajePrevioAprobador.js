...
import {
  getDetalleViajePrevioAprobador,
  aprobarViajePrevioAprobador,
  rechazarViajePrevioAprobador,
  agregarComentarioViajePrevioAprobador,
  editarComentarioViajePrevioAprobador,
  eliminarComentarioViajePrevioAprobador,
} from '../services/aprobadorService'

function useDetalleViajePrevioAprobador(id_viaje) {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      setLoading(true)
      const data = await getDetalleViajePrevioAprobador(id_viaje)
      setLoading(false)
      if (data.error) {
  ...
  const handleAprobar = async () => {
    ...
    const data = await aprobarViajePrevioAprobador(id_viaje)
    setLoadingAccion(false)
    if (data.error) { mostrarError(data.error); return }
    setAccionCompletada('EN_CURSO')
    setDatos(prev => ({ ...prev, viaje: { ...prev.viaje, estado: 'EN_CURSO' } }))
  }
  ...
  const handleRechazar = async () => {
    ...
    setLoadingAccion(true)
    const data = await rechazarViajePrevioAprobador(id_viaje)
    setLoadingAccion(false)
    if (data.error) { mostrarError(data.error); return }
  ...
  const handleAgregarComentario = async () => {
    ...
    setLoadingAccion(true)
    const data = await agregarComentarioViajePrevioAprobador(id_viaje, texto.trim())
    setLoadingAccion(false)
    if (data.error) {
    ...
    setObservaciones([''])
    const updated = await getDetalleViajePrevioAprobador(id_viaje)
  ...
  const handleConfirmarEdicion = async () => {
    ...
    setLoadingAccion(true)
    const data = await editarComentarioViajePrevioAprobador(id_viaje, comentarioEditando, textoEdicion.trim())
    setLoadingAccion(false)
  ...
  const handleConfirmarEliminacion = async () => {
    setLoadingAccion(true)
    const data = await eliminarComentarioViajePrevioAprobador(id_viaje, comentarioEliminando)
    setLoadingAccion(false)
    if (data.error) { mostrarErrorModal(data.error); return }
...
export default useDetalleViajePrevioAprobador;